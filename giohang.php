...
<head>
  ...
  <title>Giỏ hàng - Berry & Brew</title>
 ...
 <script>
  ...
  function loadCart() {
    ...
    if (cart.length === 0) {
      ...
      body.innerHTML = `
        <tr>
          <td colspan="6" style="text-align:center; padding:20px; color:#d6336c;">
      ...
      checkoutBtn.textContent = "Không thể thanh toán";
      return;
    }
  ...
  const quantity = Number(item.quantity) || 1; // nếu chưa có quantity thì mặc định 1
  ...
  body.innerHTML += `
    <tr>
      ...
      <td><button onclick="xoaSanPham(${index})" class="btn-delete">Xóa</button></td>
    </tr>
  `;
});


    total.textContent = tong.toLocaleString();
